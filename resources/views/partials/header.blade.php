<header style="background: linear-gradient(90deg,#667eea 0%,#764ba2 100%); padding: 20px;">
    <nav style="display:flex;justify-content:space-between;align-items:center;max-width:1100px;margin:auto;">
        <a href="{{ route('home') }}" style="color:white;font-size:1.8em;font-weight:bold;text-decoration:none;">News Aggregator</a>
        <ul style="list-style:none;display:flex;gap:24px;margin:0;padding:0;">
            <li><a href="{{ route('local-news') }}" style="color:white;text-decoration:none;">Local News</a></li>
            <li><a href="{{ route('international-news') }}" style="color:white;text-decoration:none;">International News</a></li>
            <li><a href="{{ route('summarizer') }}" style="color:white;text-decoration:none;">Summarizer</a></li>
            <li><a href="{{ route('news-submission') }}" style="color:white;text-decoration:none;">Submit News</a></li>
</ul>

    </nav>
</header>
